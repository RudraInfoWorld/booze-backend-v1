...
const createNotification = async (notificationData) => {
  try {
    ...
    const [settings] = await db.query('SELECT * FROM notification_settings WHERE user_id = ?', [
      userId,
    ]);
    ...
    const [notification] = await db.query('SELECT * FROM notifications WHERE id = ?', [
      notificationId,
    ]);
...
const registerDeviceToken = async (userId, deviceToken, deviceType) => {
  try {
    ...
    const [existingToken] = await db.query('SELECT id FROM device_tokens WHERE device_token = ?', [
      deviceToken,
    ]);

    if (existingToken) {
      // Update existing token
      await db.query('UPDATE device_tokens SET user_id = ?, device_type = ? WHERE id = ?', [
        userId,
        deviceType,
        existingToken.id,
      ]);
...
const unregisterDeviceToken = async (deviceToken) => {
  try {
    await db.query('DELETE FROM device_tokens WHERE device_token = ?', [deviceToken]);
